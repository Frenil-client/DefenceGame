...
원피스 랜덤 디펜스와 히어로즈 랜덤 디펜스 계보의 랜덤 디펜스다. 흔함(1성) 유닛은 계열당 1종씩 6종이며 덱 선택은 없다(6종뿐이라 선택이 무의미해 폐기, SPEC 2-3/BALANCE 1장). 뽑기로 지급되는 유닛을 합성해 상위 유닛을 만들어, 루프를 순회하는 몬스터를 처리한다. 조합이 막히는 문제(특정 재료가 안 뜸)는 선택권(석상 파괴/보스 보상으로 얻어 상점에서 원하는 1성을 사는 재화)이 통제한다. 10의 배수 라운드(10/20/30/40)가 보스이며, 보스를 제한시간 내에 처리하지 못하거나 이전 보스를 못 잡은 채 다음 보스가 나오면 패배한다. 또한 살아있는 필드 몬스터가 누적 상한(임시 60)을 초과해도 패배한다(폐기했다가 재도입).
...
| Docs/UNIT_SKILLS.md | 스킬 분배 규칙과 42종 분배 표, 5성 균형 근거 | 유닛 스킬, 전투 효과 관련 작업 |
...
7. **맵 검증기 등 공유 판정 로직은 Core에 한 벌만 존재한다.** UI, 린터, 시뮬레이터가 같은 구현을 공유한다. 따로 구현하면 반드시 갈라진다 (덱 도달 계산기는 덱 시스템 폐기로 함께 폐기됨)
...
STEP 1(기반 도구)과 STEP 2(뼈대)는 완료했다. 유닛 이동/전투/투사체는 결정적 시뮬에서 들어내 Unity 실시간으로 처리한다(전투/타겟팅/이동은 CombatController, UnitMoveController가 담당하고, 시뮬은 배치/스폰/순회/코스트만 유지). 웨이브 제한시간과 보스(10의 배수) 승패도 도입했다. 시뮬/검증은 기능 구현 이후 필요할 때 사후 추가하며, 시뮬 제약이 기능을 막지 않는다.

현재는 STEP 3(뽑기와 조합의 재미 판정) 구간이다. 덱 시스템은 없으므로 뽑기와 조합, 그리고 선택권(랜덤성 통제)으로 재미를 판정한다. 이 판정 전까지 아트, 사운드, 연출에 시간을 쓰지 않는다. **히어로 시스템(STEP 4)으로 이 판정을 미루지 않는다.** 히어로는 재미있는 조합 위에 얹는 것이지 재미없는 조합을 구제하는 장치가 아니다.
