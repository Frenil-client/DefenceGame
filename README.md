...
스킬은 `Data/skills.csv`에 발동 조건과 효과로 정의하고 `Data/units.csv`의 `skillIds`로 유닛에 배정합니다. 그 정의가 실제 전투에서 어떻게 보이는지를 아래에 모았습니다.

### 발동 조건

![버서커가 평타를 반복하다가 네 번째 타격에서만 굵은 주황색 빔을 쏘는 장면](Docs/images/skill-trigger.gif)

버서커의 스킬은 `[강타] 평타 4회마다 피해 100% 추가`입니다. 평타는 가는 노란 빔이고 배수가 걸린 타격은 굵은 주황 빔이라, 네 번에 한 번씩 굵어지는 것이 보입니다. 피해 숫자를 띄우는 대신 타격 자체를 구분합니다.

### 효과의 종류

| 광역 | 다중 타격 |
|---|---|
| ![룬나이트가 공격할 때 명중 지점에 주황색 원이 그려지는 장면](Docs/images/skill-area.png) | ![닌자가 공격할 때 두 갈래 빔이 서로 다른 적에게 뻗는 장면](Docs/images/skill-multi.png) |
| `[광역 2] 명중 지점 반경 2 안의 적에게 50% 피해`. 명중 지점에 반경이 그려집니다. 청록 원은 공격 사거리라 둘이 겹쳐 보입니다 | `[다중 타격] 가까운 2명을 동시에 타격`. 실제로 맞은 대상마다 빔이 한 갈래씩 나갑니다 |

효과가 다르면 표시도 달라야 무엇이 작동했는지 알 수 있습니다. 반경 안의 적을 한 번에 때리는 것과 여러 대상을 각각 때리는 것은 화면 결과가 비슷해 보여도 다른 계산입니다.

### 오라

![스킬이 없는 1성 법사의 공격력이 12 (+2)이고 적용 중 버프에 전투 함성이 표시된 화면](Docs/images/skill-aura-2.png)

법사는 1성이라 스킬이 없습니다. 그런데 공격력이 `12 (+2)`이고 적용 중 버프에 `[전투 함성] 공격력 15% 상승`이 붙어 있습니다. 근처 다른 유닛의 오라를 받고 있기 때문입니다.

같은 스킬 ID의 오라는 몇 기가 겹쳐도 한 번만 더해지고, ID가 다르면 각각 더해집니다. 반경 밖으로 나가면 증가분과 목록이 함께 사라집니다.
