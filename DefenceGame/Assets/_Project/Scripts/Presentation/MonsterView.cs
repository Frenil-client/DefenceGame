...
namespace Synthesis.Presentation
{
    ...
    // 인스펙터 확인 + HP바 앵커. 상태는 Core(LoopMonster)가 소유한다.
    public sealed class MonsterView : MonoBehaviour, IPoolable
    {
        ...
        private long maxHpRaw = 1;

        public void Bind(LoopMonster m)
        {
            monster = m;
            enemyId = m != null ? m.enemyId : "";
            maxHpRaw = (m != null && m.hp.raw > 0) ? m.hp.raw : 1;
            if (hpBar != null) hpBar.ResetFull();
        }

        // 매 프레임 라이브 값 갱신(인스펙터 + HP바).
        public void Refresh()
        {
            if (monster == null) return;
            waypointIndex = monster.waypointIndex;
            if (hpBar != null) hpBar.SetRatio((float)monster.hp.raw / maxHpRaw);
        }
