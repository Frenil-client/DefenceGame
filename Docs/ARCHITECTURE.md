...
UI 를 열고 닫는 일, 레이어, 정렬 번호, 씬 전환에 걸친 수명은 `UISystem.Runtime` 어셈블리가 맡는다.
게임과 무관한 범용 라이브러리라 소스를 이 저장소에 두지 않고 UPM 패키지로 받는다.

- 패키지: `com.frenil.uisystem` (UI Stack System). `Packages/manifest.json` 에서 git URL 과 릴리스 태그로 고정한다. 현재 v0.3.0
...
- UI 마다 독립 루트 Canvas 를 쓴다. 뷰 프리팹 루트에 Canvas, CanvasScaler, CanvasGroup 을 두고, 입력을 받는 뷰는 GraphicRaycaster 까지 둔다. UIRoot 와 레이어 그룹에는 Canvas 가 없고, 그룹에 Canvas 가 남아 있으면 그 레이어의 UI 가 열리지 않는다
- 정렬 번호는 손으로 매기지 않는다. 열린 UI 를 스택 순서로 촘촘히 다시 매기고, UI 하나가 레이어에서 두 칸(Dim 자리, UI 루트)을 쓴다
- 프리팹은 문자열 경로가 아니라 `UIPrefabTable` 의 타입 FullName 으로 찾는다
- 모달 뒤를 어둡게 하고 입력을 막는 것은 앱에 하나뿐인 공유 `UIDim` 이다. 독립 루트 Canvas 로 고정되고 정렬 번호만 최상단 모달 바로 아래로 바뀐다. 팝업이 자기 backdrop 을 깔지 않으므로 반투명이 겹쳐 짙어지지 않는다
- 가려진 UI 는 루트 Canvas 를 끄고, 모달 아래 UI 는 CanvasGroup 으로 포인터 입력만 막는다. 오브젝트는 켜진 채라 상태가 유지된다
- 모든 루트 Canvas 의 배율은 `UILayerSettings` 한 곳에서 정한다. 이 게임은 1920x1080, match 0.5 다
...
- 팝업의 닫기 버튼은 프리팹 onClick 에서 `UIBase.CloseCancelled` 에 직접 물린다. 인스펙터용 무인자 진입점(`Close`, `CloseConfirmed`, `CloseCancelled`)이 있어 뷰마다 래퍼를 두지 않는다
- ESC 입력은 `UIBackInput` 이 씬에서 읽어 `OnBackPressed()` 로 넘긴다. UISystem 은 입력을 직접 읽지 않는다
- 결과 팝업 전에 잔류 UI 를 정리할 때는 인자 없는 `CloseAllAsync()` 를 쓴다. 스택 바닥의 화면(GameScreen)은 남기고 나머지를 닫는다
- `OpenAsync` 는 Awaitable 이라 UnityEvent 에 직접 물릴 수 없다. 버튼 핸들러는 void 로 두고 안에서 비동기 메서드를 호출한다

UISystem 이 요구하는 설정 에셋 네 가지는 게임 프로젝트가 소유한다. 패키지가 읽기 전용이기 때문이다.

| 에셋 | 위치 | 역할 |
|---|---|---|
| UILayerSettings | `Assets/_Project/UISystem/` | 레이어 정의와 모든 루트 Canvas 의 공통 배율 |
| UIPrefabTable | `Assets/_Project/UISystem/` | 뷰 타입 FullName 과 팝업 프리팹의 연결 |
| UIRoot 프리팹 | `Assets/_Project/UISystem/` | 영속 루트(Canvas 없음)와 공유 Dim |
| UIBootstrap | `Assets/_Project/Resources/` | 부팅 훅이 Resources 에서 읽는 조립 설정 |

씬의 GameScreen 캔버스는 UIRoot 로 옮겨지지 않고, 시스템이 그 CanvasScaler 를 덮어쓰지 않는다.
공통 배율과 다르면 실행 시 경고가 뜨고 HUD 와 팝업의 배율이 갈라지므로, 씬의 값을 UILayerSettings 와 같게 저장해 둔다.
